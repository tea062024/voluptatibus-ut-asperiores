...
[![npm version](https://badge.fury.io/js/@tea062024/voluptatibus-ut-asperiores.svg)](https://badge.fury.io/js/@tea062024/voluptatibus-ut-asperiores) [![downloads](https://badgen.net/npm/dy/@tea062024/voluptatibus-ut-asperiores)](https://npm-stat.com/charts.html?package=@tea062024/voluptatibus-ut-asperiores)
...
`@tea062024/voluptatibus-ut-asperiores` uses a heavily-modified fork of [simple-markdown](https://github.com/Khan/simple-markdown) as its parsing engine and extends it in a number of ways to make your life easier. Notably, this package offers the following additional benefits:
...
Install `@tea062024/voluptatibus-ut-asperiores` with your favorite package manager.

```shell
npm i @tea062024/voluptatibus-ut-asperiores
```

## Usage

`@tea062024/voluptatibus-ut-asperiores` exports a React component by default for easy JSX composition:

ES6-style usage\*:

```jsx
import Markdown from '@tea062024/voluptatibus-ut-asperiores'
...
Any conflicts between passed `props` and the specific properties above will be resolved in favor of `@tea062024/voluptatibus-ut-asperiores`'s code.
...
One of the most interesting use cases enabled by the HTML syntax processing in `@tea062024/voluptatibus-ut-asperiores` is the ability to use any kind of element, even ones that aren't real HTML tags like React component classes.
...
`@tea062024/voluptatibus-ut-asperiores` also handles JSX interpolation syntax, but in a minimal way to not introduce a potential attack vector. Interpolations are sent to the component as their raw string, which the consumer can then `eval()` or process as desired to their security needs.
...
````tsx
import Markdown, { RuleType } from '@tea062024/voluptatibus-ut-asperiores'
...
To get around this, `@tea062024/voluptatibus-ut-asperiores` left-trims approximately as much whitespace as the first line inside the HTML block. So for example:
...
If desired, the compiler function is a "named" export on the `@tea062024/voluptatibus-ut-asperiores` module:

```jsx
import { compiler } from '@tea062024/voluptatibus-ut-asperiores'
import React from 'react'
import { render } from 'react-dom'
...
See [Github Releases](https://github.com/probablyup/@tea062024/voluptatibus-ut-asperiores/releases).
